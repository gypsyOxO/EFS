...
import Disclaimers from "components/Form/Upload/Disclaimers"
...
const SelectCommType = ({ comm, index }) => {
	...
	const listOfCommtypes = "getCommtypes" in data ? getDistinctOptions(data.getCommtypes, "COMM_TYPE", "COMM_TYPE_DESC") : null
	...
	const selectedCommtype =
		"getCommtypes" in data ? (comm.COMM_TYPE !== "" ? data.getCommtypes.find(commtype => commtype.COMM_TYPE === comm.COMM_TYPE) : "") : ""

	const DisplayUploadsAndDisclaimers = (
		<Fragment>
			<UploadCommType index={index} {...selectedCommtype} />
			<Disclaimers {...selectedCommtype} {...comm} index={index} />
        </Fragment>        
    )
	...
	return (
		<Fragment>
            ...
            {selectedCommtype && DisplayUploadsAndDisclaimers}
		</Fragment>
	)
}
